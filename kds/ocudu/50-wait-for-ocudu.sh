...
NAMESPACE="ocudu"
LABEL="app.kubernetes.io/name=ocudu-gnb"
TIMEOUT="300s"

echo "Waiting for gNB Pod ($LABEL) in namespace $NAMESPACE..."
...
if oc wait --for=condition=Ready pod -l "$LABEL" -n "$NAMESPACE" --timeout="$TIMEOUT"; then
    echo "---------------------------------------------------"
    echo "✅ SUCCESS: The gNB Pod is now RUNNING and READY!"
    echo "---------------------------------------------------"
    oc get pods -n "$NAMESPACE" -l "$LABEL"
else
    echo "---------------------------------------------------"
    echo "❌ FAILURE: The gNB Pod failed to reach Ready state within $TIMEOUT."
    echo "---------------------------------------------------"
